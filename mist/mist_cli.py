...
#         self._reset_info()
    ...
    def _print_greeting(self):
        ...
        print self._format_string('Benvenuto in %s versione %s'  % (mist_messages.SWN, __version__), frame = frame, color = bcolors.BLUE)
        print self._format_string('', frame = frame)
        print self._format_string('Premendo il tasto C avvierai la profilazione della macchina per la misura.', frame = frame, color = bcolors.YELLOW)
        print self._format_string('', frame = frame)
        print self._format_string('Premendo il tasto M avvierai una profilazione e il test di misura completo.', frame = frame, color = bcolors.GREEN)
        print self._format_string('', frame = frame)
